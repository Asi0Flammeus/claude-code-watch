...
def install_statusline(verbose: bool = True) -> Tuple[bool, str]:
    ...
    script_content = STATUSLINE_SCRIPT_CONTENT.replace("claude-watch", claude_watch_path)
    ...
    try:
        ...
        if verbose:
            print(f"{Colors.GREEN}✓ Created status line script{Colors.RESET}")
    ...
    try:
        ...
        if verbose:
            print(f"{Colors.GREEN}✓ Updated Claude Code settings{Colors.RESET}")
    ...
    if verbose:
        ...
        print(f"  {Colors.YELLOW}[⚠ S:80% W:45%]{Colors.RESET} - Warning (75-90%)")
        print(f"  {Colors.RED}[🔴 S:95% W:67%]{Colors.RESET} - Critical (>90%)")
        print()
