...
if [ "$SESSION_INT" -ge 95 ]; then
    if [ "$SILENT" != "1" ]; then
        echo "⚠️  Claude usage CRITICAL: ${SESSION_INT}% - Consider pausing"
    fi
    if [ "$BLOCKING" = "1" ]; then
        exit 2
    fi
    ...
    if [ "$SILENT" != "1" ]; then
        echo "⚠️  Claude usage high: ${SESSION_INT}%"
    fi
    exit 1
fi
...
def install_hook() -> bool:
    ...
    # Handle both string and dict hook formats
    hook_exists = False
...
def run_generate_hook(install: bool = False) -> int:
    ...
    if install:
        ...
        try:
            ...
            print(f"  {Colors.CYAN}CLAUDE_USAGE_THRESHOLD{Colors.RESET}  Warning threshold (default: 85)")
            print(f"  {Colors.CYAN}CLAUDE_WATCH_SILENT{Colors.RESET}     Suppress output (set to 1)")
            print(f"  {Colors.CYAN}CLAUDE_WATCH_BLOCKING{Colors.RESET}   Block at 95%+ (set to 1)")
            print()
            print("To uninstall, remove the hook from ~/.claude/settings.json")
        except Exception as e:
            print(f"{Colors.RED}Error installing hook: {e}{Colors.RESET}", file=sys.stderr)
            return 1
    else:
...
Examples:
  ...
  claude-watch --generate-hook  Print hook script for Claude Code
  claude-watch --install-hook   Install hook to ~/.claude/settings.json
...
""",
    ...
    # Handle --generate-hook and --install-hook modes
